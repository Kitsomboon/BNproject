...
function About8() {
  ...
  return (
    <div className="container md:w-[1000px] mx-auto flex-row md:flex justify-between py-8 ">
      {/* Original Card Section - Show on Right */}
      <div
        className="relative group rounded-3xl shadow-xl shadow-gray-300 w-[auto] md:w-[500px] md:h-[500px] p-8 bg-white h-auto z-10 
        transition-all duration-200 ease-in-out hover:border-[#d1543c] hover:border-2 border-2 border-[#ffffff] md:ml-auto order-2"
      >
        {/* Header Section */}
      ...
      {/* Cards Section - Show Below on Mobile */}
      {showCards && (
        <div className="flex flex-col items-center space-y-4 py-2 mx-auto md:order-1 md:mr-8 md:hidden">
      ...
      {/* Cards Section - Show on Left on Desktop */}
      {showCards && (
        <div className="hidden md:flex flex-col items-center space-y-4 py-2 mx-auto md:order-1 md:mr-8">
          {cardData.map((card, index) => (
            <div
              key={index}
              className="rounded-3xl shadow-lg shadow-gray-300 w-[350px] md:w-[450px] p-4 bg-[#16171f] h-auto z-8 transition-all duration-200 ease-in-out border-2 border-gray-200"
